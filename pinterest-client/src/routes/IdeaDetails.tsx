...
import { ideaDetailMorePinQuery, ideaDetailsQuery } from "../utils/data";
...
export default function IdeaDetails() {
  ...
  const fetchPinDetails = () => {
    const query = ideaDetailsQuery(ideaId);
    ...
    if (query) {
      client.fetch(query).then((data) => {
        ...
        if (data[0]) {
          const query = ideaDetailMorePinQuery(data[0]);
